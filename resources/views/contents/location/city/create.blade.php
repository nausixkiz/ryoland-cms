...
@section('content')
    <div class="row">
        <div class="col-md-12 col-12">
            <form class="form form-vertical" method="POST" action="{{ route('location.cities.store') }}"
...
@section('page-script')
    <script>
        (function (window, document, $) {
            ...
            const thumbnail = $('#city-thumbnail');
            if (thumbnail.length) {
                thumbnail.change(function () {
                    ...
                    if (/^image/.test(files[0].type)) { // only image file
                        ...
                        reader.onloadend = function () {
                            $('#city-thumbnail-text').text(thumbnail.val());
                            $('#city-thumbnail-preview').attr('src', event.target.result);
                        }
                    }
                });
